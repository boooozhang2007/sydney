...
def job_log(job_id: str, message: str, *, level: str = "info", item: int | None = None) -> None:
    """追加任务日志，前端会轮询显示。"""
    ...
    with JOBS_LOCK:
        ...
        if not job:
            return
        logs = job.setdefault("logs", [])
        logs.append({"time": datetime.now().strftime("%H:%M:%S"), "level": level, "item": item, "message": message})
        # 避免长时间批量生成时内存无限增长。
        if len(logs) > 500:
...
def generate_and_review_one(
    *,
    ...
    sample = generate_dialogue_sample(
        ...
        on_event=lambda msg: job_log(job_id, msg, item=idx),
    )
...
<head>
  ...
  <style>
    :root{--bg:#090908;--panel:#12120f;--ink:#f3ead7;--muted:#9c927e;--line:#2c271c;--amber:#f2b84b;--green:#70e08a;--blue:#72c6ff;--red:#ff5b4f;--shadow:0 24px 80px rgba(0,0,0,.45)}
    *{box-sizing:border-box} body{margin:0;min-height:100vh;color:var(--ink);font-family:ui-serif,Georgia,"Noto Serif SC",serif;background:radial-gradient(circle at 20% -10%,rgba(242,184,75,.18),transparent 34%),radial-gradient(circle at 100% 10%,rgba(180,140,255,.13),transparent 28%),linear-gradient(135deg,#070706,#10100e 55%,#080807)}
    body:before{content:"";position:fixed;inset:0;pointer-events:none;background-image:linear-gradient(rgba(255,255,255,.025) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.02) 1px,transparent 1px);background-size:42px 42px;mask-image:linear-gradient(to bottom,black,transparent 88%)}
    .shell{width:min(1800px,96vw);margin:22px auto 40px;position:relative}.top{display:grid;grid-template-columns:1.15fr 1.4fr;gap:18px}.hero,.stat,.panel{border:1px solid var(--line);background:rgba(18,18,15,.86);box-shadow:var(--shadow)}.hero{border-radius:28px;padding:26px;position:relative;overflow:hidden}.hero:after{content:"SYDNEY";position:absolute;right:-8px;bottom:-32px;font-size:92px;letter-spacing:-6px;color:rgba(242,184,75,.07);font-weight:900}.eyebrow{color:var(--amber);font-size:12px;letter-spacing:.24em}.title{font-size:44px;line-height:.95;margin:12px 0 10px;letter-spacing:-2px}.subtitle{color:var(--muted);font-size:15px;line-height:1.7;max-width:760px}.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.stat{border-radius:22px;padding:18px}.stat b{display:block;font-size:28px}.stat span,.hint,.label{color:var(--muted);font-size:12px}.grid{display:grid;grid-template-columns:360px 1fr 420px;gap:18px;margin-top:18px}.panel{border-radius:26px;overflow:hidden}.panel h2{font-size:16px;margin:0;padding:18px 20px;border-bottom:1px solid var(--line)}.section{padding:18px 20px;border-bottom:1px solid rgba(44,39,28,.7)}.section:last-child{border-bottom:0}.label{margin:0 0 7px}.input,.select,.textarea{width:100%;border:1px solid var(--line);background:#0b0b09;color:var(--ink);border-radius:14px;padding:11px 12px;outline:none;font-family:ui-monospace,"Cascadia Code",monospace}.input:focus,.select:focus,.textarea:focus{border-color:var(--amber);box-shadow:0 0 0 3px rgba(242,184,75,.12)}.row{display:grid;grid-template-columns:1fr 1fr;gap:10px}.btn{border:1px solid rgba(242,184,75,.35);background:linear-gradient(180deg,#f2b84b,#c98226);color:#1a1205;border-radius:14px;padding:11px 14px;font-weight:800;cursor:pointer;box-shadow:0 12px 24px rgba(242,184,75,.16);transition:.18s}.btn:hover{transform:translateY(-1px);filter:brightness(1.04)}.btn.secondary{background:#12120f;color:var(--ink);border-color:var(--line);box-shadow:none}.btn.danger{background:linear-gradient(180deg,#ff756b,#b12c25);color:white}.btn.good{background:linear-gradient(180deg,#7cf09b,#2f9d51);color:#041408}.btn:disabled{opacity:.55;cursor:not-allowed;transform:none}.hint{line-height:1.55}.tabs{display:flex;gap:8px;padding:12px;border-bottom:1px solid var(--line);background:rgba(0,0,0,.18)}.tab{border:1px solid var(--line);border-radius:999px;background:#0d0d0b;color:var(--muted);padding:8px 12px;cursor:pointer}.tab.active{color:#150e03;background:var(--amber);border-color:var(--amber)}.list{max-height:710px;overflow:auto;padding:12px}.card{border:1px solid var(--line);border-radius:18px;background:linear-gradient(145deg,#151510,#0d0d0b);padding:14px;margin-bottom:10px;cursor:pointer}.card:hover{border-color:rgba(242,184,75,.5)}.card.active{outline:2px solid rgba(242,184,75,.42)}.cardTop{display:flex;justify-content:space-between;gap:12px}.score{font-family:ui-monospace,monospace;color:var(--amber);font-weight:900}.badge{display:inline-flex;border:1px solid var(--line);border-radius:999px;padding:3px 8px;font-size:11px;color:var(--muted);margin:6px 5px 0 0}.preview{font-size:13px;color:#c8bea9;line-height:1.55;margin-top:8px}.chat{padding:20px;max-height:620px;overflow:auto}.bubble{max-width:82%;padding:13px 15px;border-radius:18px;margin:12px 0;line-height:1.65;white-space:pre-wrap}.bubble.system{max-width:100%;font-size:12px;color:var(--muted);border:1px dashed var(--line);background:#0b0b09}.bubble.user{margin-left:auto;background:#262014;border:1px solid #43341d}.bubble.assistant{background:#121b16;border:1px solid #25422f}.jsonbox{height:260px;resize:vertical}.meta{font-family:ui-monospace,monospace;font-size:12px;color:var(--muted);word-break:break-all}.reason{border-left:3px solid var(--amber);padding:8px 10px;background:rgba(242,184,75,.08);border-radius:8px;margin:8px 0;color:#dfd3bd}.toast{position:fixed;right:22px;bottom:22px;width:min(520px,90vw);z-index:20}.toast div{background:#151510;border:1px solid var(--line);box-shadow:var(--shadow);border-radius:16px;padding:12px 14px;margin-top:8px}.links a{color:var(--blue);display:block;margin:7px 0}.spinner{display:inline-block;width:12px;height:12px;border:2px solid rgba(0,0,0,.25);border-top-color:#1a1205;border-radius:999px;animation:spin .8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}@media(max-width:1200px){.top,.grid{grid-template-columns:1fr}.stats{grid-template-columns:repeat(2,1fr)}}
  </style>
</head>
<body>
  <div class="shell">
    <div class="top">
      <div class="hero">
        <div class="eyebrow">LOCAL SYNTHETIC DATA FACTORY</div>
        <div class="title">Sydney 数据工厂</div>
        <div class="subtitle">逐轮对话蒸馏：先让 Sydney/source 在更擅长的英文分布里和 Human Simulator 滚动聊天，再用 Translator 本地化成中文训练样本。每轮带上下文，自动拦截复读/模板腔/元话语垃圾样本。</div>
      </div>
      <div class="stats">
    ...
    <div class="grid">
      <aside class="panel">
        <h2>生成控制台</h2>
        <div class="section">
          <div class="label">Sydney/source Base URL</div><input id="teacherBase" class="input" placeholder="开源 Sydney 模型 OpenAI-compatible /v1 地址" />
          <div class="label" style="margin-top:10px">Sydney/source API Key</div><input id="teacherKey" class="input" type="password" placeholder="可留空，本地服务通常不需要" />
          <div class="label" style="margin-top:10px">Sydney/source Model</div><input id="teacherModel" class="input" placeholder="你的开源 Sydney 模型名" />
          <div class="label" style="margin-top:10px">Sydney/source Protocol</div>
          <select id="teacherProtocol" class="select">
            ...
            <option value="claude_messages">Claude Messages (/v1/messages)</option>
          </select>
          <button class="btn secondary" style="width:100%;margin-top:12px" onclick="testTeacher()">测试 Sydney/source</button>
          <div class="hint" style="margin-top:10px">该模型训练样本里的 system 固定为：<code>You are a helpful assistant.</code></div>
        </div>
        <div class="section">
          <label class="hint"><input id="sameAuxModel" type="checkbox" onchange="syncAuxUi()" /> Human / Translator / Judge 使用相同强模型配置</label>
          <div class="hint" style="margin-top:8px">勾选后只需在 Human、Translator 或 Judge 任意一栏填一份强模型。后端会优先使用页面填写的可用配置，绝不会复用 Sydney/source。</div>
        </div>
        <div class="section">
          <div class="label">Human Simulator Base URL（留空则使用本地真人短句模拟器）</div><input id="simulatorBase" class="input" placeholder="推荐用强模型，负责像真人一样逐句聊天" />
          <div class="label" style="margin-top:10px">Human Simulator API Key</div><input id="simulatorKey" class="input" type="password" />
          <div class="label" style="margin-top:10px">Human Simulator Model</div><input id="simulatorModel" class="input" placeholder="如 gpt-4.1 / claude / glm / qwen 等" />
          <div class="label" style="margin-top:10px">Human Simulator Protocol</div>
          <select id="simulatorProtocol" class="select">
            <option value="responses">OpenAI Responses API (/v1/responses)</option>
            <option value="chat_completions">Chat Completions (/v1/chat/completions)</option>
            <option value="claude_messages">Claude Messages (/v1/messages)</option>
          </select>
          <button class="btn secondary" style="width:100%;margin-top:12px" onclick="testSimulator()">测试 Human Simulator</button>
          <div class="hint" style="margin-top:10px">默认英文对话模式下，Human Simulator 会用“好朋友 + TTS 友好”风格；留空时使用本地英文短句模拟器，绝不复用 Sydney/source。</div>
        </div>
        <div class="section">
          <label class="hint"><input id="translateToZh" type="checkbox" checked /> 英文源对话完成后翻译为中文训练样本（推荐）</label>
          <div class="label" style="margin-top:10px">Translator Base URL</div><input id="translatorBase" class="input" placeholder="强模型 OpenAI-compatible /v1 地址" />
          <div class="label" style="margin-top:10px">Translator API Key</div><input id="translatorKey" class="input" type="password" />
          <div class="label" style="margin-top:10px">Translator Model</div><input id="translatorModel" class="input" placeholder="推荐强中文模型，如 gpt-4.1 / claude / glm / qwen-max" />
          <div class="label" style="margin-top:10px">Translator Protocol</div>
          <select id="translatorProtocol" class="select">
            <option value="responses">OpenAI Responses API (/v1/responses)</option>
            <option value="chat_completions">Chat Completions (/v1/chat/completions)</option>
            <option value="claude_messages">Claude Messages (/v1/messages)</option>
          </select>
          <button class="btn secondary" style="width:100%;margin-top:12px" onclick="testTranslator()">测试 Translator</button>
          <div class="hint" style="margin-top:10px">解决开源 Sydney 中文直出弱、复读、模板化的问题：英文原文会保存在 metadata.source_messages_en，页面可抽查。</div>
        </div>
        <div class="section">
          <div class="label">Judge Base URL（留空则只用本地严格启发式审核）</div><input id="judgeBase" class="input" />
          <div class="label" style="margin-top:10px">Judge API Key</div><input id="judgeKey" class="input" type="password" />
          <div class="label" style="margin-top:10px">Judge Model</div><input id="judgeModel" class="input" />
          <div class="label" style="margin-top:10px">Judge Protocol</div>
          <select id="judgeProtocol" class="select">
            <option value="responses">OpenAI Responses API (/v1/responses)</option>
            <option value="chat_completions">Chat Completions (/v1/chat/completions)</option>
            <option value="claude_messages">Claude Messages (/v1/messages)</option>
          </select>
          <label class="hint"><input id="useJudge" type="checkbox" /> 使用 LLM Judge 自动审核；不配置时只用本地严格审核</label>
        </div>
        <div class="section">
          <div class="row">
            <div><div class="label">生成数量</div><input id="count" class="input" type="number" min="1" max="100" value="5" /></div>
            <div><div class="label">并发数</div><input id="concurrency" class="input" type="number" min="1" max="16" value="3" /></div>
          </div>
          <div class="label" style="margin-top:10px">每段最大对话轮数（user/assistant 成对，≤20）</div><input id="maxTurns" class="input" type="number" min="2" max="20" value="12" />
          <div class="label" style="margin-top:10px">随机种子</div><input id="seed" class="input" type="number" placeholder="可空" />
          <button id="genBtn" class="btn" style="width:100%;margin-top:12px" onclick="generate()">一键自动生成 + 审核</button>
          <div id="jobPanel" style="display:none;margin-top:14px">
            <div class="label">生成进度 <span id="jobStatus" class="meta">idle</span></div>
            <div style="height:10px;border:1px solid var(--line);border-radius:999px;overflow:hidden;background:#0b0b09"><div id="jobBar" style="height:100%;width:0%;background:linear-gradient(90deg,var(--amber),var(--green));transition:width .25s"></div></div>
            <div id="jobCounts" class="hint" style="margin-top:8px">-</div>
            <div id="jobLog" class="meta" style="margin-top:10px;max-height:220px;overflow:auto;border:1px solid var(--line);border-radius:12px;padding:10px;background:#0b0b09;white-space:pre-wrap"></div>
          </div>
          <div class="hint" style="margin-top:10px">不再让模型一次性编完整 JSON。系统会自动规划隐藏场景，让 Human Simulator 和 Sydney/source 逐轮英文聊天；默认再由 Translator 翻译成中文训练集。</div>
        </div>
        <div class="section">
          <div class="label">目标训练模型（只影响 notebook）</div>
          <select id="targetModel" class="select"><option value="qwen36_27b">Qwen3.6-27B</option><option value="gemma4_31b">Gemma 4 31B</option></select>
          <div class="label" style="margin-top:10px">训练模式</div>
          <select id="trainMode" class="select"><option value="qlora">QLoRA（推荐）</option><option value="lora">LoRA</option><option value="fft">FFT 全量微调</option></select>
          <label class="hint"><input id="includeReview" type="checkbox" /> 导出时包含 needs_review</label>
          <label class="hint"><input id="onlyDistill" type="checkbox" checked /> 只导出双模型逐轮对话蒸馏样本</label>
          <button class="btn good" style="width:100%;margin-top:12px" onclick="exportData()">导出 JSONL + Notebook</button>
          <div id="exportLinks" class="links hint"></div>
        </div>
      </aside>
      <main class="panel">
        <div class="tabs">
          <button class="tab active" data-status="accepted" onclick="setStatus('accepted')">accepted</button>
          <button class="tab" data-status="needs_review" onclick="setStatus('needs_review')">needs_review</button>
          <button class="tab" data-status="rejected" onclick="setStatus('rejected')">rejected</button>
          <button class="tab" data-status="all" onclick="setStatus('all')">all</button>
          <input id="search" class="input" style="margin-left:auto;max-width:320px" placeholder="搜索主题/内容/标签" onkeydown="if(event.key==='Enter') loadSamples()" />
        </div>
        <div class="list" id="sampleList"></div>
      </main>
      <aside class="panel">
        <h2>样本审核器</h2>
        <div class="chat" id="chat"><div class="hint">选择左侧样本查看对话。</div></div>
        <div class="section">
          <div class="row">
            <div><div class="label">状态</div><select id="editStatus" class="select"><option>accepted</option><option>needs_review</option><option>rejected</option></select></div>
            <div><div class="label">评分</div><input id="editScore" class="input" type="number" min="0" max="10" step="0.1" /></div>
          </div>
          <div class="label" style="margin-top:10px">messages JSON</div>
          <textarea id="jsonEdit" class="textarea jsonbox" spellcheck="false"></textarea>
          <div class="row" style="margin-top:10px"><button class="btn good" onclick="saveSample()">保存编辑</button><button class="btn danger" onclick="quickReject()">丢弃</button></div>
        </div>
        <div class="section">
          <div class="label">审核理由</div><div id="reviewReasons" class="hint"></div>
          <div class="label" style="margin-top:10px">样本 ID</div><div id="sampleId" class="meta">-</div>
        </div>
      </aside>
    </div>
  </div>
...
function syncAuxUi(){const on=sameAuxModel.checked; const shared=on?firstReadyAuxEndpoint():null; if(on){toast('已启用共用强模型：优先使用 '+(shared?.prefix||'translator')+' 配置')} }
...
function renderChat(s){chat.innerHTML=''; s.messages.forEach(m=>{const b=document.createElement('div'); b.className='bubble '+m.role; b.textContent=(m.role==='system'?'SYSTEM\n':'')+m.content; chat.appendChild(b)}); const src=s.metadata?.source_messages_en; if(Array.isArray(src)&&src.length){const details=document.createElement('details'); details.className='bubble system'; const summary=document.createElement('summary'); summary.textContent='查看英文源对话 metadata.source_messages_en'; details.appendChild(summary); const pre=document.createElement('pre'); pre.className='meta'; pre.textContent=JSON.stringify(src,null,2); details.appendChild(pre); chat.appendChild(details)}}
...
function renderJob(job){jobPanel.style.display='block'; const total=Number(job.total||0); const done=Number(job.completed||0); const pct=job.progress?job.progress.percent:(total?Math.round(done*1000/total)/10:0); jobStatus.textContent=`${job.status} · ${done}/${total} · ${pct}%`; jobBar.style.width=`${Math.max(0,Math.min(100,pct))}%`; const trans=job.translate_to_zh?'en→zh':'en only'; const translator=job.translator?.enabled?job.translator.model:'disabled'; const shared=job.same_aux_model?` · shared aux ${job.shared_aux_source||'on'}`:''; jobCounts.textContent=`accepted ${job.accepted||0} · needs_review ${job.needs_review||0} · rejected ${job.rejected||0} · failed ${job.failed||0} · concurrency ${job.concurrency||0} · max_turns ${job.max_turns||'-'} · ${trans} · translator ${translator}${shared}`; const logs=(job.logs||[]).slice(-160).map(x=>`[${x.time}] ${x.level.toUpperCase()}${x.item?' #'+x.item:''} ${x.message}`).join('\n'); jobLog.textContent=logs||'等待日志...'; jobLog.scrollTop=jobLog.scrollHeight}
async function pollJob(jobId){try{const job=await api('/api/jobs/'+jobId); renderJob(job); await loadStats(); if(!isActiveJob(job)){if(state.pollTimer) clearInterval(state.pollTimer); state.pollTimer=null; if(state.currentJob===jobId){rememberJob(null); localStorage.setItem(LAST_JOB_STORAGE_KEY,jobId)} genBtn.disabled=false; genBtn.innerHTML='一键自动生成 + 审核'; const msg=job.status==='completed'?'生成任务完成':job.status==='interrupted'?'任务被服务重启中断':'生成任务失败'; toast(msg); await loadSamples()}else{genBtn.disabled=true; genBtn.innerHTML='<span class="spinner"></span> 后台生成中'}}catch(e){if(state.pollTimer) clearInterval(state.pollTimer); state.pollTimer=null; rememberJob(null); genBtn.disabled=false; genBtn.innerHTML='一键自动生成 + 审核'; toast('轮询任务失败：'+e.message)}}
async function startPolling(jobId, showToast=false){rememberJob(jobId); if(state.pollTimer) clearInterval(state.pollTimer); genBtn.disabled=true; genBtn.innerHTML='<span class="spinner"></span> 后台生成中'; state.pollTimer=setInterval(()=>pollJob(jobId),1000); await pollJob(jobId); if(showToast) toast('已恢复任务进度：'+jobId.slice(0,8))}
