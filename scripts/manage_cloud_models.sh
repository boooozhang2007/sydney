...
setup_qwen(){
  ...
  log "Qwen params: port=$QWEN_PORT parallel=$QWEN_PARALLEL ctx=$QWEN_CTX_SIZE served=$QWEN_SERVED_MODEL_NAME"
  # shellcheck disable=SC2086
  env HF_ENDPOINT="$HF_ENDPOINT" GITHUB_PROXY_PREFIX="$GITHUB_PROXY_PREFIX" \
...
start_qwen(){
  ...
  log "启动 Qwen GGUF: host=$QWEN_HOST port=$QWEN_PORT parallel=$QWEN_PARALLEL ctx=$QWEN_CTX_SIZE"
  # shellcheck disable=SC2086
